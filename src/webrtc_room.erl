...
-export([create/1, delete/1, join/3, get_members/1, leave/2, rooms/0]).

create(Name) ->
    WebrtcRoomMnesia = #webrtc_room{uuid = binary_to_list(util:uuid()), name = Name},
    F = fun() ->
        mnesia:write(WebrtcRoomMnesia)
    end,
    mnesia:transaction(F).

delete(RoomUuid) when is_list(RoomUuid) ->
    Fun = fun() ->
...
join(RoomUuid, Pid, NickName) when is_list(RoomUuid), is_pid(Pid) ->
...
get_members(RoomUuid) when is_list(RoomUuid) ->
    Fun = fun() ->
...
leave(RoomUuid, Pid) when is_list(RoomUuid), is_pid(Pid) ->
    Fun = fun() ->
